...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    $new_payment_status = trim($_POST['new_payment_status']);
    ...
    if ($rental_id_to_update && $car_id_of_rental && in_array($new_status, $allowed_statuses) && in_array($new_payment_status, $allowed_payment_statuses)) {
        try {
            ...
            if (($old_status === 'booked' && ($new_status === 'cancelled' || $new_status === 'completed')) || ($old_status === 'cancelled' && $new_status === 'booked')) {
                ...
                } elseif ($new_status === 'booked' && $old_status === 'cancelled') {
                     $stmt_car_update = $pdo->prepare("UPDATE cars SET quantity = GREATEST(0, quantity - 1) WHERE id = ?");
                     $stmt_car_update->execute([$car_id_of_rental]);
                     $stmt_car_update = $pdo->prepare("UPDATE cars SET is_available = IF(quantity > 0, 1, 0) WHERE id = ?");
                }
                 if (isset($stmt_car_update)) {
                    $stmt_car_update->execute([$car_id_of_rental]);
                 }
            }
        ...
        $_SESSION['admin_action_error'] = "Invalid data provided for status update.";
        header("Location: " . APP_URL . "admin/manage_rentals.php");
        exit();
    }
}
...
try {
    ...
    $rentals = [];
    $error_message_page = "Could not fetch rentals: " . htmlspecialchars($e->getMessage());
}
?>
                    ...
                    <?php else: ?>
                        <?php foreach ($rentals as $rental): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="badge <?php echo ($rental['status'] == 'booked' ? 'bg-info text-dark' : ($rental['status'] == 'completed' ? 'bg-success' : 'bg-danger')); ?>">
                                        <?php echo htmlspecialchars(ucfirst($rental['status'])); ?>
                                    </span>
                                </td>
                                <td>
                                     <span class="badge <?php echo ($rental['payment_status'] == 'pending' ? 'bg-warning text-dark' : ($rental['payment_status'] == 'paid' ? 'bg-success' : 'bg-primary')); ?>">
                                        ...
                                        <div class="modal-content">
                                          <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                            ...
                                            <div class="modal-header">
                                              <h5 class="modal-title" id="updateStatusModalLabel-<?php echo $rental['id']; ?>">Update Rental #<?php echo $rental['id']; ?> Status</h5>
                                              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                              <div class="mb-3">
                                                <label for="new_status-<?php echo $rental['id']; ?>" class="form-label">Rental Status</label>
                                                <select class="form-select" id="new_status-<?php echo $rental['id']; ?>" name="new_status">
                                              ...
                                              <div class="mb-3">
                                                ...
                                                <select class="form-select" id="new_payment_status-<?php echo $rental['id']; ?>" name="new_payment_status">
